              ...
              @endif
                <div class="card-header">
                  <strong class="card-title">{{ $table_name }}</strong>
                    {{ Form::open(['method'=>'GET', 'url'=>['/back/user/create'], 'style'=>'display:inline' ]) }}
          ...
          <table id="bootstrap-data-table" class="table table-striped table-bordered">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td style="vertical-align:middle">{{ $row->surname }}</td>
                <td style="vertical-align:middle">{{ $row->email }}</td>
                <td style="text-align: center; vertical-align:middle">

                    {{ Form::open(['method'=>'GET', 'url'=>['/back/user/edit/' .$row->id], 'style'=>'display:inline' ]) }}
                    {{ Form::submit('Edit', ['class'=>'btn btn-primary']) }}
                    {{ Form::close() }}
                    @if ($i > 1)
                      {{ Form::open(['method'=>'DELETE', 'url'=>['/back/user/delete/' .$row->id], 'style'=>'display:inline' ]) }}
